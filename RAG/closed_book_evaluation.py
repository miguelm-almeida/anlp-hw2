...
def run_closed_book_inference(model_name, model_type, questions, output_file, batch_size=8):
    """
    ...
    for i in tqdm(range(0, len(questions), batch_size), desc="Processing Batches"):
        ...
        empty_chunks = [[{"text": "", "similarity": 0.0}] for _ in batch_questions]
        
        # Get answers
        if len(batch_questions) == 1:
            # Single question mode
            answers.append(qa_system.get_answer(batch_questions[0], empty_chunks[0]))
        else:
            # Batch mode
